...
class MDate extends MHelperBase
{
	...
	public function _timeAgo($datetime=null, $options=array(), $returnData=false)
	{
		...
		if (is_array($options)) {
			...
			if (isset($options['backward_string'])) {
				$backwardString = $options['backward_string'];
			}
		} else {
			$short = $options;
		}
		...
		if ($diff > abs($now - strtotime($end))) {
			...
			if (!$backwards) {
				$relativeDate = sprintf("%s {$backwardString}", $relativeDate);
			}
		}
		return self::_return($relativeDate, $returnData);
	}
	...
	public function _daysInMonth($month=null, $year=null, $returnData=false)
	{
		...
		return self::_return(date('t', mktime(0,0,0,$month,1,$year)), $returnData);
	}
